...
def get_aligned_msg(msg):
    ...
    return msg + " " * (max_count - msg_width)

url="https://www.ptt.cc/bbs/Python/M.1629433719.A.118.html"
...
try:
    with open("data.txt",mode="a+",encoding="utf-8") as file:
        ...
        while x >=0: # 抓整篇留言，若只抓100則 可改為 x<=99
            print("{:<3}".format(tag[x].text),"{:<14}".format(id[x].text),get_aligned_msg(message[x].text),get_aligned_msg(time[x].text.split("\n")[0]))
            file.write("{:<3}".format(tag[x].text)+"{:<14}".format(id[x].text)+get_aligned_msg(message[x].text)+get_aligned_msg(time[x].text.split("\n")[0])+"\n")
